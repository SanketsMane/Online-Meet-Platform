...
function initClient() {
    // Check for light mode from landing page
    const globalTheme = localStorage.getItem('theme');
    if (globalTheme === 'light') {
        localStorageSettings.theme = 10;
        lS.setSettings(localStorageSettings);
    } else if (globalTheme === 'dark' && localStorageSettings.theme === 10) {
        localStorageSettings.theme = 1;
        lS.setSettings(localStorageSettings);
    }
    setTheme();
...
function setTheme() {
    ...
    switch (theme) {
        ...
        case 'yellow':
            ...
            break;
        case 'light':
            swalBackground = 'rgba(255, 255, 255, 1)';
            document.documentElement.setAttribute('data-theme', 'light');
            // Variables are handled by Root.css [data-theme='light'] selector
            // But we need to set these to override potential previous inline styles or ensure consistency
            document.documentElement.style.setProperty('--body-bg', 'radial-gradient(#ffffff, #e6e6e6)');
            document.documentElement.style.setProperty('--trx-bg', 'radial-gradient(#ffffff, #e6e6e6)');
            document.documentElement.style.setProperty('--msger-bg', 'radial-gradient(#ffffff, #e6e6e6)');
            document.documentElement.style.setProperty('--left-msg-bg', '#e6e6e6');
            document.documentElement.style.setProperty('--right-msg-bg', '#dcf8c6');
            document.documentElement.style.setProperty('--select-bg', '#f8f9fa');
            document.documentElement.style.setProperty('--tab-btn-active', '#e6e6e6');
            document.documentElement.style.setProperty('--settings-bg', 'radial-gradient(#ffffff, #e6e6e6)');
            document.documentElement.style.setProperty('--wb-bg', 'radial-gradient(#ffffff, #e6e6e6)');
            document.documentElement.style.setProperty('--btns-bg-color', 'rgba(255, 255, 255, 0.7)');
            document.documentElement.style.setProperty('--dd-color', '#333333');
            document.body.style.background = 'radial-gradient(#ffffff, #e6e6e6)';
...
function showAbout() {
    ...
    Swal.fire({
        ...
        title: BRAND.about?.title && BRAND.about.title.trim() !== '' ? BRAND.about.title : 'WebRTC SFU v2.1.06',
        html: `
            <br />
            <div id="about">
                ${
