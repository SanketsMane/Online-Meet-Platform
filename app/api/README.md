...
Create a meeting with a `HTTP request` containing the `API_KEY` sent to Kidokool’s server. The response contains a `meeting` URL that can be `embedded` in your client within an `iframe`.
...
api: {
    // app/api
    keySecret: 'kidokoolsfu_default_secret',
}
```
...
<iframe
    ...
    src="https://sfu.kidokool.com/join/your_room_name"
...
<iframe
    allow="camera; microphone; display-capture; fullscreen; clipboard-read; clipboard-write; web-share; autoplay"
    src="https://sfu.kidokool.com/newroom"
    style="height: 100vh; width: 100vw; border: 0px;"
></iframe>
```
