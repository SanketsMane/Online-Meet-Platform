...
// Host Configuration
const hostCfg = config?.security?.host || { protected: false, user_auth: false };

// Widget Configuration
const widget = config?.ui?.brand?.widget || { enabled: false, alert: { enabled: false } };
...
// We will need to check if the request comes from our own frontend (session/cookie) or external (api key)
...
// Author: Sanket - Declare io at module level for global access
let io;
...
function startServer() {
    ...
    // Author: Sanket - Initialize socket.io with the HTTP server to handle WebSocket connections
    io = socketIo(server, {
    ...
    */
    ...
    app.get('/brand', async (req, res) => {
        try {
            ...
            if (settings.logo_url && branding.site) {
                //Sanket v2.0 - Pass logo_url as a top-level key so Brand.js can swap img src
            ...
            if (settings.favicon_url) {
                //Sanket v2.0 - Pass favicon_url separately so Brand.js updates <link> href not logo img
    ...
    app.get('/join/', async (req, res) => {
        if (Object.keys(req.query).length > 0) {
            ...
            // http://localhost:3010/join?room=test&name=kidokoolsfu&audio=0&video=0&screen=0&notify=0&chat=1
            // http://localhost:3010/join?room=test&roomPassword=0&name=kidokoolsfu&audio=1&video=1&screen=0&hide=0&notify=1&duration=00:00:30
            // http://localhost:3010/join?room=test&roomPassword=0&name=kidokoolsfu&audio=1&video=1&screen=0&hide=0&notify=0&token=token
    ...
    // Author: Sanket - Admin portal page for super admin to manage developers and system
    ...
    // ####################################################
    // ADMIN API ROUTES - Author: Sanket
    // ####################################################
    ...
    async function verifyAdminToken(req, res, next) {
        try {
            ...
            try {
                ...
                log.debug('[AdminAuth] Token decoded successfully:', JSON.stringify(decoded));
            } catch (error) {
            ...
            log.debug('[AdminAuth] Admin verified successfully');
            req.admin = decoded;
            next();
        } catch (error) {
            log.error('[AdminAuth] Token verification error:', error);
            return res.status(401).json({ message: 'Invalid token' });
        }
    }
    ...
    app.get(restApi.basePath + '/activeRooms', (req, res) => {
        ...
        log.debug('Kidokool get active rooms', { activeRooms });
    });
    ...
    // Middleware for legacy secret check
    ...
    // Middleware for Dual Auth (Secret OR Database Key)
    const apiAuth = async (req, res, next) => {
        if (req.headers.authorization === config.api.keySecret) {
            return next();
        }
    ...
    // Stats
    app.get(restApi.basePath + '/stats', checkApiSecret, (req, res) => {
        try {
            ...
            const stats = api.getStats(roomList);
    ...
    // Meetings
    app.get(restApi.basePath + '/meetings', checkApiSecret, (req, res) => {
        try {
            if (restApi.allowed && !restApi.allowed.meetings) {
                return res.status(403).json({ error: 'Endpoint disabled' });
            }
            const api = new ServerApi(req.headers.host, req.headers.authorization);
            res.json({ meetings: api.getMeetings(roomList) });
        } catch (error) {
            res.status(500).json({ error: 'Internal Error' });
        }
    });
    ...
    // Join (Dual Auth)
    app.post(restApi.basePath + '/join', apiAuth, (req, res) => {
        const api = new ServerApi(req.headers.host, config.api.keySecret);
    ...
    function getServerConfig(tunnel = false) {
        const safeConfig = {
            api: {
                apiKey: config.api?.keySecret || 'kidokool_sfu_secret',
            },

            // Network & Connectivity
            // Author: Sanket - Fixed undefined 'host' and 'trustProxy' variables by using config values
            network: {
                server_listen: `${config.server.listen.ip}:${config.server.listen.port}`,
            ...
            // Author: Sanket - Fixed undefined corsOptions and jwtCfg by using config values
            security: {
                cors: config.server.cors,
                jwtCfg: config.security.jwt,
            ...
            // API & Services
            // Author: Sanket - Fixed undefined webhook by using config value
            api: {
                rest_api: restApi,
                webhook: config.features?.webhook?.enabled ? config.features.webhook : false,
            },
            ...
            // Author: Sanket - Fixed undefined sentryEnabled by using config value
            monitoring: {
                sentry: config.integrations?.sentry?.enabled ? config.integrations.sentry : false,
    ...
    function encodeToken(token) {
        ...
        // Author: Sanket - Use JWT_EXPIRATION from environment variables
        const expireValue = expire || process.env.JWT_EXPIRATION || '1h';
        ...
        // Author: Sanket - Use JWT_SECRET from environment variables
        const encryptedPayload = CryptoJS.AES.encrypt(payloadString, process.env.JWT_SECRET).toString();

        // Constructing JWT token
        const jwtToken = jwt.sign({ data: encryptedPayload }, process.env.JWT_SECRET, { expiresIn: expireValue });
